...
export function Blog({ t }: { t: (s: string) => string }) {
  ...
  <div className="mt-2 flex justify-center sm:justify-end">
    <a href="/blog" className="btn-pill btn-ghost">{t(COPY.blogVoirTout)}</a>
  </div>
  {items.length === 0 ? (
  ...
  <article className="rounded-2xl border border-slate-200/60 bg-white/20 backdrop-blur-sm p-6 dark:border-white/10 dark:bg-white/5">
                {"cover" in p && p.cover && (
                ...
                <a href={`/blog/${slugify(p.title)}`} className="mt-4 inline-flex items-center gap-2 rounded-2xl border border-slate-300/70 bg-white/50 backdrop-blur-sm px-4 py-2 text-sm text-slate-800 hover:bg-white/80 transition dark:border-white/20 dark:bg-white/10 dark:text-white dark:hover:bg-white/20">
