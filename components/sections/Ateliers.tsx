...
export function Ateliers({ t }: { t: (s: string) => string }) {
  ...
  <div className="mx-auto max-w-screen-2xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-2 flex justify-center sm:justify-end">
          <a href="/ateliers" className="btn-pill btn-ghost">
            {t(COPY.ateliersVoirTout)} <ArrowRight className="btn-icon" />
          </a>
        </div>
              ...
              <TiltCard className="rounded-2xl border border-slate-300 bg-white/80 backdrop-blur-sm p-0 shadow-sm dark:border-white/10 dark:bg-white/5">
                <article>
                  ...
                  <div className="mt-4 flex gap-2">
                    <a href={`/ateliers/${slugify(a.title)}`} className="px-4 py-2 rounded-xl bg-[rgb(var(--edu-primary))] text-slate-900 font-semibold hover:bg-[#f5cd43] transition">{t(COPY.ateliersCtaDetails)}</a>
